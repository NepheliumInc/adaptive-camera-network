...
namespace graph{
	...
	protected:
		...
		//map < base-node-id, vector < vector < pair < ajecent-node-id, weight > > > >
		//map<string, map<int, EdgedNode>> Edges;
	...
	public:
		Node();
		Node(string _Id, string _Type, string _IP, string _Location);
		~Node();

		string Id;
		int ThreadId;
		string Type; // start, end, door, normal
		string IP;
		string Location; //physical
		vector<int> PossibleProfiles;
	};
